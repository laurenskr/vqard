...
class VcardDownloadController
{
    public function __invoke(string $slug): Response
    {
        ...
        if ($mobile = $f('mobile'))  $lines[] = 'TEL;TYPE=CELL:' . $mobile;
        if ($phone = $f('phone'))    $lines[] = 'TEL;TYPE=WORK:' . $phone;
        if ($email = $f('email'))    $lines[] = 'EMAIL:' . $email;
        if ($website = $f('website')) $lines[] = 'URL:' . $website;
        if ($org = $f('org'))        $lines[] = 'ORG:' . $org;
